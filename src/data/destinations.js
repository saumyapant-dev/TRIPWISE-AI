...
export const COUNTRIES_LIST = Array.from(
  new Set(DESTINATIONS_DATA.map((d) => d.country))
).sort();

/**
 * Normalizes text for accent-insensitive and fuzzy searching.
 */
...
/**
 * Resolves destination information from city name or full display name.
 */
export function findDestinationByCity(cityName) {
  if (!cityName || typeof cityName !== "string") return null;
  const clean = normalizeStr(cityName);

  // Exact city match
  const exact = DESTINATIONS_DATA.find((d) => normalizeStr(d.city) === clean);
  if (exact) return exact;

  // Exact city + country match
  const withCountry = DESTINATIONS_DATA.find(
    (d) => `${normalizeStr(d.city)}, ${normalizeStr(d.country)}` === clean
  );
  ...
  return (
    DESTINATIONS_DATA.find(
      (d) =>
        clean.includes(normalizeStr(d.city)) ||
        (normalizeStr(d.city).includes(clean) && clean.length >= 3)
    ) || null
  );
}

/**
 * Filter destinations by country and query text across city, region, and country.
 */
export function filterDestinations({ country = "", search = "" }) {
  let list = DESTINATIONS_DATA;
  if (country && country !== "ALL") {
    const targetCountry = normalizeStr(country);
    list = list.filter((d) => normalizeStr(d.country) === targetCountry);
  }

  if (search && search.trim()) {
    const q = normalizeStr(search);
    list = list.filter((d) => {
      const c = normalizeStr(d.city);
      const reg = normalizeStr(d.region || "");
      const cnt = normalizeStr(d.country);
      return c.includes(q) || reg.includes(q) || cnt.includes(q);
    });

    // Rank prefix matches higher
    list.sort((a, b) => {
      const aStarts = normalizeStr(a.city).startsWith(q) ? 1 : 0;
      const bStarts = normalizeStr(b.city).startsWith(q) ? 1 : 0;
